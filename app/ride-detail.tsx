...
import {
  ...
  type KeyMarker,
  type SpeedDataPoint,
} from "@/components/speed-curve-chart";
...
import {
  buildLocalActivityHighlights,
  calculateBestPowerEfforts,
} from "@/lib/local-activity-insights";
...
import {
  attachRidePhotos,
  loadRidePhotoTimeline,
  removeRidePhoto,
  type RidePhotoTimelineEntry,
} from "@/lib/local-ride-photos";
...
const ACTIVITY_VIEWER_DRAWER_COLLAPSED_HEIGHT = Math.min(
  Math.round(SCREEN_H * 0.46),
  360,
);
const ACTIVITY_VIEWER_DRAWER_EXPANDED_HEIGHT = Math.min(
  Math.round(SCREEN_H * 0.78),
  620,
);
const ACTIVITY_VIEWER_STAGE_COLLAPSED_HEIGHT =
  SCREEN_H - ACTIVITY_VIEWER_DRAWER_COLLAPSED_HEIGHT;
const ACTIVITY_VIEWER_STAGE_EXPANDED_HEIGHT =
  SCREEN_H - ACTIVITY_VIEWER_DRAWER_EXPANDED_HEIGHT;
const ACTIVITY_DETAIL_MAIN_HERO_HEIGHT =
  ACTIVITY_VIEWER_STAGE_COLLAPSED_HEIGHT + 20;

function buildStoredActivityStatistics(record: RideRecord) {
  const movingTimeSec =
    record.movingTime ??
    Math.max(0, record.duration - (record.totalPausedSec ?? 0));
  const hasPowerEvidence =
    record.powerSource !== "unavailable" &&
    (record.avgPower > 0 ||
      record.maxPower > 0 ||
      (record.powerHistory?.some((power) => power > 0) ?? false));
  const powerSampleDurationSec = hasPowerEvidence ? movingTimeSec : 0;
  const powerWorkJ =
    (record.totalWorkKj ?? (record.avgPower * movingTimeSec) / 1000) * 1000;
...
function clampActivityViewerDrawerHeight(value: number): number {
  return Math.min(
    ACTIVITY_VIEWER_DRAWER_EXPANDED_HEIGHT,
    Math.max(ACTIVITY_VIEWER_DRAWER_COLLAPSED_HEIGHT, value),
  );
}
...
function activityTypeLabel(
  value: RideRecord["activityType"],
  t: (key: string) => string,
): string {
  return value
    ? t(`detail.activityTypes.${value}`)
    : t("detail.activityTypes.other");
}
...
export default function RideDetailScreen() {
  ...
  const { state, updateRecordName, updateRideActivity, getRouteStats } =
    useRide();
  const [isEditModalVisible, setIsEditModalVisible] = useState(false);
  const [editDescInput, setEditDescInput] = useState("");
  const [editNameInput, setEditNameInput] = useState("");
  const [localMedia, setLocalMedia] = useState<string[]>([]);
  const [editCoverPhotoUri, setEditCoverPhotoUri] = useState<
    string | undefined
  >(undefined);
  const [editActivityType, setEditActivityType] =
    useState<RideRecord["activityType"]>("road");
  ...
  const [activityViewerMode, setActivityViewerMode] = useState<
    "route" | "photos"
  >("route");
  const activityViewerRef = useRef<ScrollView>(null);
  const activityViewerDrawerHeight = useRef(
    new Animated.Value(ACTIVITY_VIEWER_DRAWER_COLLAPSED_HEIGHT),
  ).current;
  const activityViewerDrawerExpandedRef = useRef(false);
  const activityViewerDrawerDragStartHeight = useRef(
    ACTIVITY_VIEWER_DRAWER_COLLAPSED_HEIGHT,
  );

  const rideSplits = useMemo(
    () => (record ? buildRideSplits(record) : []),
    [record],
  );
  const sportType = record?.sportType ?? "cycling";
  const elevationBands = useMemo(
    () =>
      record && (sportType === "hiking" || sportType === "trail_running")
        ? buildElevationBands(record)
        : [],
    [record, sportType],
  );

  const storePickedPhotos = useCallback(
    async (assets: ImagePicker.ImagePickerAsset[]) => {
      if (!record || assets.length === 0) return;
      const timeline = await attachRidePhotos(
        record.id,
        assets.map((asset) => ({
          uri: asset.uri,
    ...
    void ImagePicker.getPendingResultAsync()
      .then((result) => {
        if (active && result && "canceled" in result && !result.canceled)
          void storePickedPhotos(result.assets);
      })
      .catch(() => undefined);
    return () => {
      active = false;
  ...
  const handleRemoveRidePhoto = useCallback(
  ...
  const photoRouteMarkers = useMemo<PhotoMapMarker[]>(
    () =>
      buildPhotoRouteMarkers(photoTimeline, record?.route ?? []).map(
        (marker) => ({
          id: marker.id,
          lat: marker.latitude,
  ...
  const activityPhotos = useMemo(() => {
    const timelinePhotos = photoTimeline.map((photo) => ({
      id: photo.id,
      uri: photo.uri,
    }));
  ...
  const coverPhotoChoices = useMemo(() => {
    const timelineChoices = photoTimeline.map((photo) => ({
      id: photo.id,
      uri: photo.uri,
    }));
  ...
  const coverPhotoUri = useMemo(
    () =>
      resolveActivityCoverPhotoUri(
        record?.coverPhotoUri,
        activityPhotos.map((photo) => photo.uri),
      ),
    [activityPhotos, record?.coverPhotoUri],
  );
  const setActivityViewerDrawer = useCallback(
    (expanded: boolean) => {
  ...
  const activityViewerDrawerResponder = useRef(
    PanResponder.create({
      ...
      onMoveShouldSetPanResponder: (_event, gestureState) =>
        !activityViewerDrawerExpandedRef.current &&
      ...
      onPanResponderMove: (_event, gestureState) => {
        const nextHeight = clampActivityViewerDrawerHeight(
          activityViewerDrawerDragStartHeight.current - gestureState.dy,
        );
      ...
      onPanResponderRelease: (_event, gestureState) => {
        ...
        const expanded =
          ...
          (gestureState.vy <= 0.2 &&
            activityViewerDrawerDragStartHeight.current - gestureState.dy >
              ACTIVITY_VIEWER_DRAWER_COLLAPSED_HEIGHT + travel * 0.45);
  ...
  const openActivityViewer = useCallback(
    (index: number) => {
      ...
      if (safeIndex > 0) {
        setTimeout(
          () =>
            activityViewerRef.current?.scrollTo({
              x: (safeIndex - 1) * SCREEN_W,
              animated: false,
            }),
          80,
        );
  ...
  const openPhotoFromMarker = useCallback(
    (photoId: string) => {
  ...
  const activitySensorAnalysis = useMemo(() => {
    ...
    return buildActivitySensorAnalysis(
      record.route.map((point, index) => ({
        ...
        powerW:
          ...
          record.powerHistory?.[
            Math.min(
              record.powerHistory.length - 1,
              Math.floor(
                (index / routeLength) *
                  Math.max(0, record.powerHistory.length - 1),
              ),
            )
          ],
      ...
      })),
      {
        ...
        temperatureC:
          record.calculationProfile?.environment?.averageTemperatureC,
    ...
    estimationCalibration.confidence,
    estimationCalibration.intensityAdjustment,
    estimationCalibration.rpeSampleCount,
    record,
    settings.age,
    settings.ftp,
  ...
  const keyMarkers = useMemo(() => {
    ...
    if (record.maxSpeed > 0) {
      const maxSpeedIdx = speedCurveData.reduce(
        (maxIdx, curr, idx) =>
          curr.speed > speedCurveData[maxIdx].speed ? idx : maxIdx,
        0,
      );
      markers.push({
        type: "maxSpeed",
  ...
  const handleExportGpx = useCallback(async () => {
    ...
    try {
      ...
      if (!backup) {
        Alert.alert(
          t("detail.exportUnavailableTitle"),
          t("detail.exportGpxUnavailable"),
        );
        return;
      }
      ...
      if (canShare) {
        ...
        Alert.alert(
          ...
          t("detail.gpxBackupCreated", { filename: backup.filename }),
        );
      }
    } catch (err) {
  ...
  const handleExportFit = useCallback(async () => {
    ...
    try {
      ...
      if (canShare) {
        ...
        Alert.alert(
          t("detail.backupCreated"),
          t("detail.fitBackupCreated", { filename: backup.filename }),
        );
      }
    } catch (error) {
  ...
  const handleShare = useCallback(async () => {
    ...
    const powerSourceLabel =
      activityStats.powerSource === "measured"
        ? "功率計量測"
  ...
  const activityEstimatePowerHistory =
    activitySensorAnalysis?.points
  ...
  const displayedPower =
    ...
    activityStats.maxPowerW !== undefined
      ...
      : activityEstimatePowerHistory.length > 0
        ? {
            averagePowerW:
              activityEstimatePowerHistory.reduce(
                (sum, power) => sum + power,
                0,
              ) / activityEstimatePowerHistory.length,
  ...
  const displayedTraining = displayedPower
    ...
    : undefined;
  const powerUnit =
    displayedPower?.source === "measured"
      ? "W（量測）"
      : displayedPower?.source === "estimated"
        ? "W（本機估算）"
        : "資料不足";
  const caloriesUnit =
    activityStats.caloriesSource === "power-estimate"
      ? "kcal（功率估算）"
      : activityStats.caloriesSource === "met-estimate"
        ...
        : activityStats.caloriesSource === "mixed-estimate"
          ? "kcal（綜合估算）"
          : "資料不足";
  const activityHighlights = buildLocalActivityHighlights(
    record,
    state.records,
  );
    ...
    <View style={styles.container}>
      <View style={styles.mapHero}>
        ...
        {coverPhotoUri ? (
          <Pressable
            style={({ pressed }) => [
              styles.activityCoverPhoto,
              { opacity: pressed ? 0.86 : 1 },
            ]}
            onPress={() => {
              const index = activityPhotos.findIndex(
                (photo) => photo.uri === coverPhotoUri,
              );
              if (index >= 0) openActivityViewer(index + 1);
            }}
          >
            ...
            <View style={styles.activityCoverShade}>
              <Text style={styles.activityCoverEyebrow}>
                {t("detail.cover")}
              </Text>
              <Text style={styles.activityCoverCopy}>
                {t("detail.viewCover")}
              </Text>
            </View>
          </Pressable>
        ...
        {activityPhotos.length > 0 && (
          <Pressable
            style={({ pressed }) => [
              styles.routeMapPhotoThumbButton,
              { opacity: pressed ? 0.78 : 1 },
            ]}
            onPress={() => openActivityViewer(1)}
          >
            <Image
              source={{ uri: activityPhotos[0].uri }}
              style={styles.routeMapPhotoThumbImage}
            />
            <View style={styles.routeMapPhotoThumbBadge}>
              <IconSymbol name="photo.fill" size={13} color="#fff" />
              <Text style={styles.routeMapPhotoThumbCount}>
        ...
        <View style={styles.activityBody}>
          ...
          <View style={styles.activityDetailsAfterInitial}>
            ...
            <View style={styles.activityMetaRow}>
              <View style={styles.activityMetaChip}>
                <Text style={styles.activityMetaChipText}>
                  {activityTypeLabel(record.activityType, t)}
                </Text>
              </View>
              {record.perceivedExertion !== undefined && (
                <View
                  style={[styles.activityMetaChip, styles.activityMetaRpeChip]}
                >
                  <Text style={styles.activityMetaRpeText}>
                    {record.perceivedExertionSource === "app-estimate"
                      ? "App 推定 "
                      : ""}
                    RPE {record.perceivedExertion}/10
                  </Text>
                </View>
              )}
              {record.equipment ? (
                <Text style={styles.activityEquipment}>{record.equipment}</Text>
              ) : null}
            </View>
          </View>
          ...
          {activityHighlights.length > 0 && (
            <View style={styles.activityHighlightsCard}>
              <Text style={styles.activityHighlightsTitle}>
                {t("detail.localHighlights")}
              </Text>
              {activityHighlights.map((highlight) => {
                ...
                return (
                  <View
                    ...
                    style={styles.activityHighlightRow}
                  >
                    <View
                      style={[
                        styles.activityHighlightDot,
                        { backgroundColor: color },
                      ]}
                    />
                    <View style={{ flex: 1 }}>
                      <Text style={styles.activityHighlightTitleText}>
                        {highlight.title}
                      </Text>
                      <Text style={styles.activityHighlightDetail}>
                        {highlight.detail}
                      </Text>
                    </View>
                  </View>
          ...
          <View style={styles.activityAnalysisCard}>
            <Text style={styles.activityAnalysisTitle}>
              {t("detail.activityAnalysis")}
            </Text>
            <Text style={styles.activityAnalysisHint}>
              {t("detail.activityAnalysisHint")}
            </Text>
            {speedCurveData.length > 1 ? (
              <SpeedCurveChart
                ...
                markers={keyMarkers}
                sources={
                  activitySensorAnalysis?.sources ?? {
                    speed: "measured",
                    power: "estimated",
                    heartRate: "estimated",
                    cadence: "estimated",
              ...
              <Text style={styles.activityAnalysisEmpty}>
                {t("detail.speedCurveUnavailable")}
              </Text>
            )}
            <ActivityElevationChart route={record.route} />
            {bestPowerEfforts.length > 0 && (
              <View style={styles.bestPowerBlock}>
                <Text style={styles.bestPowerTitle}>
                  {t("detail.bestAveragePower")}
                </Text>
                <ScrollView
                  horizontal
                  showsHorizontalScrollIndicator={false}
                  contentContainerStyle={styles.bestPowerRow}
                >
                  {bestPowerEfforts.map((effort) => (
                    <View key={effort.seconds} style={styles.bestPowerChip}>
                      <Text style={styles.bestPowerDuration}>
                        {formatPowerInterval(effort.seconds)}
                      </Text>
                      <Text style={styles.bestPowerWatts}>
                        {effort.watts} W
                      </Text>
                    </View>
                  ))}
          ...
          {(record.personalBests?.length ?? 0) > 0 && (
            <View style={styles.localInsightCard}>
              ...
              <View style={styles.localInsightText}>
                <Text style={styles.localInsightTitle}>
                  {t("detail.localPersonalRecord")}
                </Text>
                <Text style={styles.localInsightCopy}>
                  {t("detail.personalRecordImproved", {
                    records: record.personalBests
                      ?.map((best) => best.label)
                      .join("、"),
                  })}
                </Text>
          ...
          <Text style={styles.moreDataHeading}>
            {t("detail.completeRideData")}
          </Text>
          <Text style={styles.moreDataHint}>
            {t("detail.completeRideDataHint")}
          </Text>
          ...
          <View style={styles.expandedContent}>
            ...
            {/* 坡度分布詳細統計 */}
            {record &&
              (record.gradeDistribution || [0, 0, 0, 0, 0, 0]).reduce(
                ...
                <View style={styles.gradeDistributionSection}>
                  ...
                  <View style={styles.tableContainer}>
                    ...
                    {[
                      ...
                      const dist =
                        (record.gradeDistribution?.[idx] || 0) / 1000;
                      const totalDist = (
                        record.gradeDistribution || [0, 0, 0, 0, 0, 0]
                      ).reduce((a: number, b: number) => a + b, 0);
                      const pct =
                        totalDist > 0
                          ? (
                              ((record.gradeDistribution?.[idx] || 0) /
                                totalDist) *
                              100
                            ).toFixed(1)
                          : "0";
                      ...
                      return (
                        <View key={`dist-${idx}`} style={styles.tableRow}>
                          <View
                            style={[
                              styles.gradeCell,
                              { backgroundColor: gradeColors[idx] + "33" },
                            ]}
                          >
                            <Text
                              style={[
                  ...
                  <View style={styles.tableContainer}>
                    ...
                    {[
                      ...
                      const totalAscent = (
                        record.gradeAscentDistribution || [0, 0, 0, 0, 0, 0]
                      ).reduce((a: number, b: number) => a + b, 0);
                      const pct =
                        totalAscent > 0
                          ? ((ascent / totalAscent) * 100).toFixed(1)
                          : "0";
                      const gradeColors = [
                        "#34C759",
                        "#FFD60A",
            ...
            {rideSplits.length > 0 && (
              ...
              >
                <Text style={[styles.panelTitle, { color: colors.foreground }]}>
                  {t("detail.perKilometerSplits")}
                </Text>
                <Text style={[styles.panelHint, { color: colors.muted }]}>
                  {t("detail.splitsHint")}
                </Text>
                <View style={styles.splitHeader}>
                  ...
                  <Text style={styles.splitHeaderText}>
                    {sportType === "running" || sportType === "trail_running"
                      ? t("detail.pace")
                      : t("dashboard.avgSpeed")}
                  </Text>
                  <Text style={styles.splitHeaderText}>
                    {t("detail.ascentDescent")}
                  </Text>
                  <Text style={styles.splitHeaderText}>
                ...
                {rideSplits.map((split) => (
                  <View key={split.index} style={styles.splitRow}>
                    <Text style={styles.splitCell}>
                      {split.distanceM >= 950
                        ? `${split.index} km`
                        : `${split.index} · ${(split.distanceM / 1000).toFixed(2)} km`}
                    </Text>
                    <Text style={styles.splitCell}>
                      {formatDuration(split.movingTimeSeconds)}
                    </Text>
                    <Text style={styles.splitCell}>
                      {sportType === "running" || sportType === "trail_running"
                        ? formatPaceSeconds(split.paceSecondsPerKm ?? 0)
                        : (split.averageSpeedKmh?.toFixed(1) ?? "--")}
                    </Text>
                    <Text
                      style={styles.splitCell}
                    >{`${Math.round(split.ascentM)} / ${Math.round(split.descentM)}`}</Text>
                    <Text style={styles.splitCell}>
                      {sportType === "trail_running"
                        ? formatPaceSeconds(
                            calculateGapPaceSecPerKm(
                              split.paceSecondsPerKm ?? 0,
                              record.averageGrade ?? 0,
                            ),
                          )
                        : sportType === "hiking"
                          ? `${split.movingTimeSeconds > 0 ? Math.round((split.ascentM / split.movingTimeSeconds) * 3600) : 0}`
                          : split.averagePowerW === undefined
                            ? "--"
                            : `${split.averagePowerW} W`}
                    </Text>
                  </View>
                ))}
              </View>
            ...
            {elevationBands.length > 0 && (
              ...
              >
                <Text style={[styles.panelTitle, { color: colors.foreground }]}>
                  {t("detail.elevationBandDistribution")}
                </Text>
                <Text style={[styles.panelHint, { color: colors.muted }]}>
                  {t("detail.elevationBandsHint")}
                </Text>
                <View style={styles.splitHeader}>
                  <Text style={styles.splitHeaderText}>
                ...
                {elevationBands.map((band) => (
                  <View key={band.label} style={styles.splitRow}>
                    ...
                    <Text style={styles.splitCell}>
                      {(band.distanceM / 1000).toFixed(2)} km
                    </Text>
                    <Text style={styles.splitCell}>
                      {formatDuration(band.movingTimeSeconds)}
                    </Text>
                    <Text style={styles.splitCell}>
                      {Math.round(band.ascentM)} m
                    </Text>
                  </View>
                ))}
              </View>
            ...
            {/* 心率區間分布 */}
            {heartRateZones &&
              heartRateZones.reduce((a, b) => a + b, 0) > 0 && (
                <View style={styles.zoneSection}>
                  <Text style={styles.sectionTitle}>
                    {t("detail.heartRateDistribution")}
                  </Text>
                  <View style={styles.zoneRow}>
                    {renderHeartRatePie()}
                    <View style={styles.zoneLegend}>
                      {HR_ZONES.map((zone, i) => {
                        ...
                        const pct =
                          total > 0
                            ? Math.round((heartRateZones[i] / total) * 100)
                            : 0;
                        return (
                          <View key={i} style={styles.zoneLegendItem}>
                            <View
            ...
            <View style={[styles.statsPanel, { borderColor: colors.border }]}>
              ...
              <View style={styles.statsGrid}>
                <DetailCell
                  label="距離"
                  value={`${(activityStats.distanceM / 1000).toFixed(2)}`}
                  unit="km"
                />
                <DetailCell
                  label="活動時間"
                ...
                <DetailCell
                  label="消耗熱量"
                  value={
                    activityStats.caloriesSource === "unavailable"
                      ? "--"
                      : `${Math.round(activityStats.caloriesKcal)}`
                  }
                  unit={caloriesUnit}
                />
                <DetailCell
                ...
                <DetailCell
                  label="自動暫停總時間"
                  value={formatDuration(record.autoPausedSec ?? 0)}
                  unit=""
                />
              </View>
            </View>
            ...
            >
              ...
              <View style={styles.statsGrid}>
                ...
                <DetailCell
                  label="平均坡度"
                  value={
                    activityStats.averageGradePct === undefined
                      ? "--"
                      : activityStats.averageGradePct.toFixed(1)
                  }
                  unit="%"
                />
                <DetailCell
                  label="最高持續坡度"
                  value={
                    record.maxGrade !== undefined
                      ? record.maxGrade.toFixed(1)
                      : "--"
                  }
                  unit={
                    record.maxGrade === undefined
            ...
            >
              <Text style={[styles.panelTitle, { color: colors.foreground }]}>
                {t("detail.advancedTraining")}
              </Text>
              <View style={styles.statsGrid}>
                <DetailCell
                  label="平均心率"
                  value={record.avgHeartRate ? `${record.avgHeartRate}` : "--"}
                  unit="bpm"
                  color="#EF4444"
                />
                <DetailCell
                  label="最大心率"
                  value={record.maxHeartRate ? `${record.maxHeartRate}` : "--"}
                  unit="bpm"
                  color="#EF4444"
                />
                <DetailCell
                  ...
                  value={
                    displayedPower === undefined
                      ? "--"
                      : `${Math.round(displayedPower.averagePowerW)}`
                  }
                  unit={powerUnit}
                  accent
                />
                <DetailCell
                  ...
                  value={
                    displayedPower === undefined
                      ? "--"
                      : `${Math.round(displayedPower.maxPowerW)}`
                  }
                  unit={powerUnit}
                  accent
                />
                <DetailCell
                  ...
                  value={
                    displayedPower === undefined
                      ? "--"
                      : `${Math.round(activityStats.totalWorkKj ?? (displayedPower.averagePowerW * movingDuration) / 1_000)}`
                  }
                  unit={
                    displayedPower?.source === "estimated"
                      ? "kJ（本機估算）"
                ...
                <DetailCell
                  label="標準化功率"
                  value={
                    displayedTraining === undefined
                      ? "--"
                      : `${Math.round(record.normalizedPower ?? displayedTraining.normalizedPower)}`
                  }
                  unit={
                    displayedTraining === undefined ? "資料不足" : powerUnit
                  }
                  accent
                />
                <DetailCell
                  label="平均踏頻"
                  value={record.avgCadence ? `${record.avgCadence}` : "--"}
                  unit="rpm"
                />
                <DetailCell
                  label="最大踏頻"
                  value={record.maxCadence ? `${record.maxCadence}` : "--"}
                  unit="rpm"
                />
              </View>
            </View>
              ...
              <View style={styles.statsGrid}>
                <DetailCell
                  ...
                  value={
                    displayedTraining === undefined
                      ? "--"
                      : `${(record.tss ?? displayedTraining.tss).toFixed(1)}`
                  }
                  unit={displayedTraining === undefined ? "資料不足" : "TSS"}
                  color="#9C27B0"
                />
                <DetailCell
                  label="強度係數"
                  value={
                    displayedTraining === undefined
                      ? "--"
                      : `${(record.intensityFactor ?? displayedTraining.intensityFactor).toFixed(2)}`
                  }
                  unit={displayedTraining === undefined ? "資料不足" : "IF"}
                />
                <DetailCell
                  label="標準化功率"
                  value={
                ...
                <DetailCell
                  ...
                  value={
                    displayedTraining === undefined
                      ? "--"
                      : displayedTraining.trainingEffect.toFixed(1)
                  }
                  unit={
                    displayedTraining === undefined
                      ? "資料不足"
                      : displayedTraining.trainingEffectLabel
                  }
                  color="#A855F7"
                />
              </View>
            ...
            {(record.personalBests?.length ?? 0) > 0 && (
              <View
                style={[
                  styles.statsPanel,
                  { borderColor: "#F59E0B66", marginTop: 12 },
                ]}
              >
                <Text style={[styles.panelTitle, { color: "#F59E0B" }]}>
                  {t("detail.localPersonalBest")}
                </Text>
                <Text
                  style={[styles.personalBestHint, { color: colors.muted }]}
                >
                  {t("detail.localPersonalBestHint")}
                </Text>
                <View style={styles.statsGrid}>
            ...
            {record.tss !== undefined && record.tss > 0 && (
              ...
              >
                <Text style={[styles.panelTitle, { color: colors.foreground }]}>
                  {t("detail.trainingLoadRecovery")}
                </Text>
                <View style={styles.statsGrid}>
                  <DetailCell
                    label="訓練負荷"
                    value={`${Math.round(record.tss * 1.5)}`}
                    unit=""
                    color="#FF6F00"
                  />
                  <DetailCell
                    label="負荷等級"
                    value={
                      record.tss > 300 ? "高" : record.tss > 150 ? "中" : "低"
                    }
                    unit=""
                  />
                  <DetailCell
            ...
            >
              <Text style={[styles.panelTitle, { color: colors.foreground }]}>
                {t("detail.supplyRecords")}
              </Text>
              <View style={styles.statsGrid}>
                <DetailCell
                  label={t("audit.sweatLoss")}
                  value={`${Math.round(record.totalSweatMl)}`}
                  unit="ml"
                  color="#4FC3F7"
                />
                <DetailCell
                  label={t("audit.refillCount")}
                  value={`${record.refillCount}`}
                  unit={t("units.count")}
                />
              </View>
            </View>
            ...
            >
              <Text style={[styles.panelTitle, { color: colors.foreground }]}>
                {t("detail.environmentSmartSupply")}
              </Text>
              <View style={styles.statsGrid}>
                <DetailCell
                  label={t("audit.averageTemperature")}
                  value={
                    record.calculationProfile?.environment
                      ?.averageTemperatureC === undefined
                      ? "--"
                      : record.calculationProfile.environment.averageTemperatureC.toFixed(
                          1,
                        )
                  }
                  unit="°C"
                ...
                />
                <DetailCell
                  label={t("audit.averageHumidity")}
                  value={
                    record.calculationProfile?.environment
                      ?.averageHumidityPct === undefined
                      ? "--"
                      : record.calculationProfile.environment.averageHumidityPct.toFixed(
                          0,
                        )
                  }
                  unit="%"
                ...
                />
                <DetailCell
                  label={t("audit.averageWindSpeed")}
                  value={
                    record.calculationProfile?.environment
                      ?.averageWindSpeedKmh === undefined
                      ? "--"
                      : record.calculationProfile.environment.averageWindSpeedKmh.toFixed(
                          1,
                        )
                  }
                  unit="km/h"
                />
                <DetailCell
                  label={t("audit.calculationSource")}
                  value={
                    record.calculationProfile?.environment?.source ===
                    "live-weather"
                      ? t("routes.currentWeather")
                      : t("routes.offlineFallback")
                  }
                  unit=""
                />
              </View>
              {(record.supplyConfirmations?.length ?? 0) > 0 ? (
                <View style={styles.confirmationList}>
                  ...
                  {record.supplyConfirmations?.slice(-4).map((entry, index) => (
                    ...
                    >
                      {entry.type === "water"
                        ? t("audit.confirmedWater", {
                            amount: entry.recommendedWaterMl ?? "--",
                          })
                        : t("audit.confirmedEnergy", {
                            energy: entry.recommendedEnergyKcal ?? "--",
                            carbohydrate:
                              entry.recommendedCarbohydrateG ?? "--",
                          })}
                      {entry.source
                        ? ` · ${entry.source === "custom" ? t("audit.fixedThreshold") : entry.source === "smart" ? t("audit.smartCalculation") : t("audit.localEnvironmentBaseline")}`
                        : ""}
                    </Text>
                  ))}
                </View>
            ...
            >
              <View style={styles.photoTimelineHeader}>
                <View style={{ flex: 1 }}>
                  ...
                  >
                    {t("audit.localPhotoTimeline")}
                  </Text>
                  <Text
                    style={[styles.photoTimelineHint, { color: colors.muted }]}
                  >
                    {t("audit.localPhotoTimelineHint")}
                  </Text>
                </View>
                <Pressable
                  style={({ pressed }) => [
                  ...
                  onPress={() => void handlePickRidePhotos()}
                >
                  <Text
                    style={[
                      styles.photoAddButtonText,
              ...
              {photoTimeline.length === 0 ? (
                <Text style={[styles.photoEmpty, { color: colors.muted }]}>
                  {t("audit.noRidePhotos")}
                </Text>
              ) : (
                <ScrollView
                  horizontal
                  showsHorizontalScrollIndicator={false}
                  contentContainerStyle={styles.photoTimelineList}
                >
                  {photoTimeline.map((photo) => (
                    <View key={photo.id} style={styles.photoCard}>
                      ...
                      <Text style={[styles.photoTime, { color: colors.muted }]}>
                        {new Date(
                          photo.capturedAt ?? photo.selectedAt,
                      ...
                      <Pressable
                        ...
                        onPress={() => void handleRemoveRidePhoto(photo.id)}
                      >
            ...
            {routeStats && (
              ...
              >
                <Text style={[styles.panelTitle, { color: colors.foreground }]}>
                  {t("audit.routeStatistics")}
                </Text>
                <View style={styles.statsGrid}>
                  <DetailCell
                    label={t("audit.rideCount")}
                    value={`${routeStats.rideCount}`}
                    unit={t("units.count")}
                  />
                  <DetailCell
                    label={t("audit.averageSpeed")}
                    value={`${routeStats.avgSpeed.toFixed(1)}`}
                    unit="km/h"
                  />
                  <DetailCell
                    label={t("audit.bestSpeed")}
                    value={`${routeStats.bestSpeed.toFixed(1)}`}
                    unit="km/h"
                  />
                  <DetailCell
                    label={t("audit.bestTime")}
                    value={formatDuration(routeStats.bestTime)}
                    unit=""
                  />
                  <DetailCell
                    label={t("audit.totalDistance")}
                    value={`${routeStats.totalDistance.toFixed(1)}`}
                    unit="km"
                  />
                  <DetailCell
                    label={t("audit.totalAscent")}
                    value={`${Math.round(routeStats.totalAscent)}`}
                    unit="m"
                  />
                </View>
              </View>
      ...
      <Modal
        visible={isActivityViewerVisible}
        animationType="fade"
        onRequestClose={closeActivityViewer}
      >
        <View style={styles.mediaViewer}>
            ...
            style={[
              styles.activityViewerStage,
              {
                height: activityViewerDrawerHeight.interpolate({
                  ...
                  outputRange: [
                    ACTIVITY_VIEWER_STAGE_COLLAPSED_HEIGHT,
                    ACTIVITY_VIEWER_STAGE_EXPANDED_HEIGHT,
                  ],
                  extrapolate: "clamp",
                }),
              },
          ...
          <Animated.View
            style={[
              styles.activityViewerDrawer,
              { height: activityViewerDrawerHeight },
            ]}
            {...activityViewerDrawerResponder.panHandlers}
          >
            <ActivitySummaryHeader
              title={record.name}
              subtitle={`${dateStr} · ${t(`sports.${record.sportType === "trail_running" ? "trailRunning" : (record.sportType ?? "cycling")}`)}`}
        ...
        <View style={styles.modalOverlay}>
          ...
          >
            ...
            >
              ...
              <View>
                <Text style={[styles.label, { color: colors.muted }]}>
                  {t("forms.activityEditor.equipmentLabel")}
                </Text>
                <TextInput
              ...
              <View>
                <Text style={[styles.label, { color: colors.muted }]}>
                  {t("forms.activityEditor.rpeHint")}
                </Text>
                    ...
                    (value) => {
                      ...
                      return (
                        <Pressable
                          key={value}
                          onPress={() =>
                            setEditRpe(selected ? undefined : value)
                          }
                          style={[
                            styles.rpeOption,
                            {
                        ...
                        >
                          <Text
                            style={{
                              color: selected ? "#FCD34D" : colors.muted,
                              fontSize: 12,
                              fontWeight: "800",
                            }}
                          >
              ...
              <View>
                <Text style={[styles.label, { color: colors.muted }]}>
                  {t("forms.activityEditor.privateNotes")}
                </Text>
                <TextInput
                  value={editDescInput}
                  onChangeText={setEditDescInput}
                  placeholder={t(
                    "forms.activityEditor.privateNotesPlaceholder",
                  )}
                  placeholderTextColor={colors.muted}
                  multiline
                  numberOfLines={3}
              ...
              <View>
                <Text style={[styles.label, { color: colors.muted }]}>
                  {t("forms.activityEditor.coverPhoto")}
                </Text>
                <Text style={[styles.coverPickerHint, { color: colors.muted }]}>
                  {t("forms.activityEditor.coverPhotoHint")}
                </Text>
                {coverPhotoChoices.length > 0 ? (
                  <ScrollView
                    horizontal
                    showsHorizontalScrollIndicator={false}
                    contentContainerStyle={styles.coverPickerRail}
                  >
                    {coverPhotoChoices.map((photo) => {
                      ...
                      return (
                        <Pressable
                          ...
                          onPress={() => setEditCoverPhotoUri(photo.uri)}
                        >
                          <Image
                            source={{ uri: photo.uri }}
                          ...
                          {selected ? (
                            <View
                              style={[
                                styles.coverPickerSelected,
                                { backgroundColor: colors.primary },
                              ]}
                            >
                              <Text
                                style={[
                                  styles.coverPickerSelectedText,
                                  { color: colors.onAccent },
                                ]}
                              >
                                {t("forms.activityEditor.coverSelected")}
                              </Text>
                            </View>
                          ) : null}
                        </Pressable>
                  ...
                  <Text
                    style={[styles.coverPickerEmpty, { color: colors.muted }]}
                  >
                    {t("forms.activityEditor.coverEmpty")}
                  </Text>
                )}
                {editCoverPhotoUri ? (
                  <Pressable
                    style={styles.clearCoverButton}
                    onPress={() => setEditCoverPhotoUri(undefined)}
                  >
                    <Text style={styles.clearCoverButtonText}>
                      {t("forms.activityEditor.clearCover")}
                    </Text>
                  </Pressable>
                ) : null}
              </View>
              <View>
                ...
                >
                  <Text style={[styles.label, { color: colors.muted }]}>
                    {t("forms.activityEditor.localMedia")}
                  </Text>
                  <Pressable
                  ...
                  >
                    ...
                    >
                      + {t("forms.activityEditor.addMedia")}
                    </Text>
                  </Pressable>
                </View>
                {localMedia.length > 0 ? (
                  ...
                  >
                    {t("forms.activityEditor.mediaEmpty")}
                  </Text>
                )}
              </View>
            </ScrollView>
...
const DETAIL_LABEL_KEYS: Record<string, string> = {
  距離: "share.distance",
  總爬升: "share.elevation",
  爬升海拔: "share.elevation",
  移動時間: "share.movingTime",
  平均速度: "share.averageSpeed",
  平均功率: "share.averagePower",
  卡路里: "share.calories",
  最高速度: "share.maxSpeed",
  最高功率: "detail.maxPower",
  活動時間: "detail.elapsedTime",
  暫停時間: "detail.pausedTime",
  平均踏頻: "detail.avgCadence",
  最大踏頻: "detail.maxCadence",
  最高海拔: "detail.maxAltitude",
  水分流失: "detail.sweatLoss",
  補水次數: "detail.refillCount",
  平均配速: "detail.avgPace",
  爬升速度: "detail.climbRate",
  自動暫停總時間: "detail.autoPausedTime",
  總爬升高度: "detail.totalAscent",
  總下降高度: "detail.totalDescent",
  最小海拔: "detail.minElevation",
  平均坡度: "detail.averageGrade",
  最高持續坡度: "detail.maxSustainedGrade",
  平均心率: "detail.avgHeartRate",
  最大心率: "detail.maxHeartRate",
  機械工作量: "detail.mechanicalWork",
  標準化功率: "detail.normalizedPower",
  訓練壓力分數: "detail.trainingStressScore",
  強度係數: "detail.intensityFactor",
  訓練效果: "detail.trainingEffect",
  訓練負荷: "detail.trainingLoad",
  負荷等級: "detail.loadLevel",
  建議恢復: "detail.recommendedRecovery",
  平均溫度: "detail.averageTemperature",
  平均濕度: "detail.averageHumidity",
  平均風速: "detail.averageWindSpeed",
  計算來源: "detail.calculationSource",
};

function translatedDetailLabel(
  t: (key: string) => string,
  label: string,
): string {
...
function CoreActivitySummaryGrid({
  ...
  const gap = formatPaceSeconds(
    calculateGapPaceSecPerKm(
      averageSpeedKmh > 0 ? 3600 / averageSpeedKmh : 0,
      averageGrade,
    ),
  );
    ...
    sportType === "running"
      ...
      : sportType === "hiking"
        ? ([
            ...
            ["總爬升", `${Math.round(ascentM)} 公尺`],
            ["移動時間", formatDuration(movingDuration)],
            ["爬升速度", `${Math.round(vam)} 公尺/小時`],
            ["最高海拔", `${Math.round(altitudeM)} 公尺`],
            ["卡路里", `${Math.round(calories)} 卡`],
          ] as const)
        : sportType === "trail_running"
          ? ([
    ...
    <View style={styles.coreActivitySummaryGrid}>
      {entries.map(([label, value]) => (
        <View key={label} style={styles.coreActivitySummaryMetric}>
          <Text style={styles.coreActivitySummaryLabel}>
            {translatedDetailLabel(t, label)}
          </Text>
  ...
  return (
    <View style={styles.activitySummaryHeader}>
      ...
      <AdaptiveFormText
        ...
        minFontScale={0.72}
        maxLinesBeforeShrink={1}
        style={styles.activitySummaryTitle}
      >
        {title}
      </AdaptiveFormText>
      <AdaptiveFormText
        ...
        style={styles.activitySummarySubtitle}
      >
        {subtitle}
      </AdaptiveFormText>
    </View>
...
const styles = StyleSheet.create({
  ...
  mapHero: {
    height: ACTIVITY_DETAIL_MAIN_HERO_HEIGHT,
    width: SCREEN_W,
    position: "relative",
    overflow: "hidden",
  },
  ...
  activityInitialSummary: {
    paddingTop: ACTIVITY_SUMMARY_CONTENT_TOP,
    paddingBottom: ACTIVITY_SUMMARY_CONTENT_BOTTOM,
  },
  ...
  activityHighlightTitleText: {
    color: "rgba(255,255,255,0.9)",
    fontSize: 13,
    fontWeight: "700",
  },
  activityHighlightDetail: {
    color: "rgba(255,255,255,0.5)",
    fontSize: 11,
    marginTop: 2,
  },
  ...
  coreActivitySummaryMetric: {
    width: "50%",
    alignItems: "center",
    paddingHorizontal: 6,
  },
  coreActivitySummaryLabel: {
    color: "rgba(255,255,255,0.72)",
    fontSize: 13,
    fontWeight: "700",
    marginBottom: 5,
  },
  coreActivitySummaryValue: {
    color: "#fff",
    fontSize: 23,
    fontWeight: "800",
    fontVariant: ["tabular-nums"],
  ...
  activityViewerDrawerMetricValue: {
    color: "#fff",
    fontSize: 16,
    fontWeight: "800",
    fontVariant: ["tabular-nums"],
  },
  activityViewerDrawerMetricLabel: {
    color: "rgba(255,255,255,0.55)",
    fontSize: 10,
    marginTop: 3,
  },
  activityViewerRouteInfo: {
    position: "absolute",
    left: 18,
    right: 18,
    bottom: 82,
  ...
  activityViewerRoutePhotoThumb: {
    width: "100%",
    height: 70,
    borderRadius: 7,
    backgroundColor: "rgba(255,255,255,0.08)",
  },
  activityViewerRoutePhotoMeta: {
  ...
  activityViewerRouteEmpty: {
    color: "rgba(255,255,255,0.58)",
    fontSize: 11,
    lineHeight: 16,
    marginTop: 10,
  },
  activityViewerPhotoInfo: {
    position: "absolute",
    left: 18,
    right: 18,
    bottom: 84,
