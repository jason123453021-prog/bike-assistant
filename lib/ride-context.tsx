...
import {
  normalizeRideRecord,
  normalizeRideRecords,
} from "./ride-record-normalizer";
import { estimateAutomaticRpe } from "@/lib/automatic-rpe";
import {
  buildRideTimeTotals,
  calculatePausedSeconds,
} from "@/lib/ride-time-accounting";
import {
...
export interface RideState {
  ...
  customSupplyItemCounts: Record<
    string,
    { count: number; lastTriggeredTime: number }
  >;
  supplyConfirmations: SupplyConfirmation[];
  laps: RideLap[];
...
type RideAction =
  ...
  | {
      type: "SYNC_AUTO_LAPS";
      laps: RideLap[];
      anchor?: Omit<RideLapAnchor, "routePointIndex">;
    }
...
export function rideReducer(state: RideState, action: RideAction): RideState {
  switch (action.type) {
    ...
    case "PAUSE":
      if (!canPauseRide(state.status)) return state;
      {
        ...
        return {
          ...
          autoPauseStartPausedSec:
            pauseSource === "automatic" ? state.autoPausedSec : null,
        };
      }
    ...
    case "LOCATION_UPDATE": {
      ...
      const effectiveIntervalSec = Math.max(
        0,
      ...
      const movingIntervalSec = shouldCountMovingTime
        ? effectiveIntervalSec
        : 0;
      const countedDistanceM = shouldCountMovingTime
        ? Math.max(0, distanceM ?? 0)
        : 0;
      const nextElapsed = state.elapsed + movingIntervalSec;
      const newPowerWorkJ =
        state.powerWorkJ + Math.max(0, power) * movingIntervalSec;
      const newPowerSampleDurationSec =
        state.powerSampleDurationSec + movingIntervalSec;
      const avgPower =
        newPowerSampleDurationSec > 0
          ? newPowerWorkJ / newPowerSampleDurationSec
          : 0;
      ...
      const mergedPowerSource: ActivityPowerSource =
        state.powerSource === "measured" || powerSource === "measured"
          ? "measured"
          : state.powerSource === "estimated" || powerSource === "estimated"
            ? "estimated"
            : "unavailable";
      const mergedCaloriesSource: ActivityCaloriesSource =
        state.caloriesSource === "unavailable"
          ? caloriesSource
          : caloriesSource === "unavailable" ||
              caloriesSource === state.caloriesSource
            ? state.caloriesSource
            : "mixed-estimate";
      const nextMinElevation =
        acceptedElevationM === undefined
          ? state.minElevation
          : state.minElevation === null
            ? acceptedElevationM
            : Math.min(state.minElevation, acceptedElevationM);
      const nextMaxElevation =
        acceptedElevationM === undefined
          ? state.maxElevation
          : state.maxElevation === null
            ? acceptedElevationM
            : Math.max(state.maxElevation, acceptedElevationM);
      ...
      return {
        ...
        avgSpeed:
          nextElapsed > 0
            ? (state.distance + distance) / 1000 / (nextElapsed / 3600)
            : 0,
        currentPower: power,
        avgPower: Math.round(avgPower),
        maxPower: Math.max(
          state.maxPower,
          Math.max(0, maxPowerCandidate ?? power),
        ),
        ...
        totalDescent:
          state.totalDescent + (shouldCountMovingTime ? descent : 0),
        ...
        totalCalories: shouldCountMovingTime
          ? newTotalCalories
          : state.totalCalories,
        distance: state.distance + distance,
        powerHistory: shouldCountMovingTime
          ? newPowerHistory
          : state.powerHistory,
        powerZones: newZones,
    ...
    case "SYNC_AUTO_LAPS": {
      ...
      return {
        ...state,
        laps: action.laps.map((lap, index) => ({
          ...lap,
          index: index + 1,
          source: "auto",
        })),
    ...
    case "SET_AUTO_PAUSED_SECONDS": {
      const merged = mergeAutoPausedSeconds(
        state.totalPausedSec,
        state.autoPausedSec,
        Math.round(action.totalSec),
      );
      return {
        ...state,
        ...merged,
...
function decimateRoute(
  ...
  for (const point of route) {
    if (point.recordedDuringPause && !result.includes(point))
      result.push(point);
  }
...
function generateDefaultName(
  ...
  const period =
    hour < 6 ? "深夜" : hour < 12 ? "早晨" : hour < 18 ? "下午" : "夜間";
...
export function RideProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    void AsyncStorage.getItem(SPORT_TYPE_STORAGE_KEY)
      .then((stored) => {
        if (
          stored === "cycling" ||
          stored === "running" ||
          stored === "hiking" ||
          stored === "trail_running"
        ) {
  ...
  const saveRecord = useCallback(
    async (name?: string, calculationProfile?: RideCalculationProfile) => {
      ...
      const elapsedDurationSec = state.startTime
        ? Math.max(
            0,
            Math.round(((state.endTime ?? now) - state.startTime) / 1000),
          )
        : state.elapsed + state.totalPausedSec;
      const pausedForActivitySec = Math.max(
        0,
        elapsedDurationSec - state.elapsed,
      );
      const timeTotals = buildRideTimeTotals(
        state.elapsed,
        pausedForActivitySec,
      );
      ...
      const recordBase: RideRecord = {
        ...
        name:
          name && name.trim()
            ? name.trim()
            : generateDefaultName(now, state.sportType),
        ...
        autoPausedSec: Math.min(
          timeTotals.totalPausedSec,
          Math.max(0, state.autoPausedSec),
        ),
        movingTime,
        // 坡度分布數據
        ...
        perceivedExertion:
          calculationProfile?.autoRpeEnabled === false
            ? undefined
            : automaticRpe.value,
      ...
      // 新騎乘的即時儀表、會話快照與 summary 都使用同一批已接受高度增量。
      // 正規化仍修補舊資料與其他欄位，但不可用抽樣後的 route 重建值覆寫本次已顯示的爬升。
  ...
  const loadRecords = useCallback(async () => {
    try {
      ...
      if (data) {
        const migrated = normalizeRideRecords(JSON.parse(data)).map(
          (record) => ({
            ...record,
            name:
              record.name || generateDefaultName(record.date, record.sportType),
          }),
        );
        if (JSON.stringify(migrated) !== data) {
          await AsyncStorage.setItem(
            STORAGE_KEY,
            JSON.stringify(migrated.slice(0, 100)),
          );
        }
  ...
  const updateRideActivity = useCallback(
    async (id: string, updates: RideActivityUpdate) => {
      try {
        const data = await AsyncStorage.getItem(STORAGE_KEY);
        if (data) {
          const records: RideRecord[] = JSON.parse(data);
            ...
            if (r.id === id) {
              return {
                ...
                ...(updates.description !== undefined
                  ...
                  : {}),
                ...(updates.mediaItems !== undefined
                  ? { mediaItems: updates.mediaItems }
                  : {}),
                ...(updates.coverPhotoUri !== undefined
                  ? { coverPhotoUri: updates.coverPhotoUri ?? undefined }
                  : {}),
                ...(updates.activityType !== undefined
                  ? { activityType: updates.activityType }
                  : {}),
                ...(updates.sportType !== undefined
                  ? { sportType: updates.sportType }
                  : {}),
                ...(updates.equipment !== undefined
                  ? { equipment: updates.equipment }
                  : {}),
                ...(updates.perceivedExertion !== undefined
                  ? { perceivedExertion: updates.perceivedExertion }
                  : {}),
                ...(updates.perceivedExertionSource !== undefined
                  ? { perceivedExertionSource: updates.perceivedExertionSource }
                  : {}),
              };
            }
            return r;
  ...
  const checkSnapshot =
    useCallback(async (): Promise<Partial<RideState> | null> => {
      try {
  ...
  const getRouteStats = useCallback(
    (routeName: string): RouteStats | null => {
      ...
      const avgSpeed =
        routeRecords.reduce((sum, r) => sum + r.avgSpeed, 0) / rideCount;
      ...
      const totalDistance = routeRecords.reduce(
        (sum, r) => sum + r.distance / 1000,
        0,
      );
      const totalAscent = routeRecords.reduce(
        (sum, r) => sum + r.totalAscent,
        0,
      );
