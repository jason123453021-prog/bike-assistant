...
export function buildPhotoRouteMarkers(entries: RidePhotoTimelineEntry[], route: LocationPoint[]): PhotoRouteMarker[] {
  ...
  return entries.flatMap<PhotoRouteMarker>((entry, index) => {
    if (isCoordinatePair(entry.latitude, entry.longitude)) {
      const timeMatched = entry.capturedAt ? nearestRoutePoint(route, entry.capturedAt) : undefined;
      return [{
        id: entry.id,
        latitude: entry.latitude,
        longitude: entry.longitude!,
        altitude: timeMatched?.altitude ?? undefined,
        label: `照片 ${index + 1}`,
    ...
    return [{
      ...
      altitude: matched.altitude ?? undefined,
      label: `照片 ${index + 1}`,
      source: "route-time" as const,
    }];
  });
}
