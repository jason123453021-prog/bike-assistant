...
describe("本機騎乘統計物理校正", () => {
  it("保留 30 m 內、1.8 km/h 以上的低速爬坡樣本，並拒絕超過精度上限的資料", () => {
    expect(MAX_RIDE_STATISTICS_ACCURACY_M).toBe(30);
    expect(MIN_CYCLING_MOVEMENT_SPEED_KMH).toBe(1.8);
    expect(hasReliableRideMovement({ speedKmh: 1.8, distanceM: 0.8, accuracyM: 30 })).toBe(true);
    expect(hasReliableRideMovement({ speedKmh: 1.7, distanceM: 1.5, accuracyM: 30 })).toBe(false);
    expect(hasReliableRideMovement({ speedKmh: 0, distanceM: 31, accuracyM: 30 })).toBe(true);
    expect(hasReliableRideMovement({ speedKmh: 1.8, distanceM: 2, accuracyM: 30.1 })).toBe(false);
  });
