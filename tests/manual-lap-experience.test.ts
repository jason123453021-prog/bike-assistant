...
describe("自動距離 Lap 體驗", () => {
  it("封存剛完成固定距離分圈，總累計不會被歸零或把 overshoot 併入圈內", () => {
    ...
    expect(result.laps[0]).toMatchObject({ index: 1, distanceM: 1_000, source: "auto" });
  ...
  it("連續自動計圈不設圈數上限，且以前景／背景完整快照同步", () => {
    ...
    expect(rideContextSource).not.toContain('laps: action.laps.map((lap, index) => ({ ...lap, index: index + 1, source: "auto" })).slice');
  });
});
