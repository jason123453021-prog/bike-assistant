...
describe("彈窗與系統通知同步清理守門", () => {
  it("清除補給通知時，同時處理已呈現及未到期排程的通知", () => {
    ...
    expect(source).not.toContain('notification.request.content.data?.type === "daylight_alert"');
    expect(source).not.toContain('notification.content.data?.type === "daylight_alert"');
  });
