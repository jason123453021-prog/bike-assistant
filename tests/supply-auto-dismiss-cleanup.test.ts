...
describe("一般補給自動關閉 timer 的資源清理", () => {
  it("以類型化 ref 集中追蹤能量與補水的自動關閉 timer", () => {
    expect(mapSource).toContain('const supplyAutoDismissTimersRef = useRef<Partial<Record<"calorie" | "water", ReturnType<typeof setTimeout>>>>({});');
    expect(mapSource).toContain('const clearSupplyAutoDismissTimer = useCallback((type?: "calorie" | "water") => {');
  });
  ...
  it("在確認、稍後、清除所有提醒與卸載時釋放 timer", () => {
    ...
    expect(mapSource).toContain('useEffect(() => () => clearSupplyAutoDismissTimer(), [clearSupplyAutoDismissTimer]);');
    expect(mapSource).toContain('clearSupplyAutoDismissTimer();\n    Object.values(supplyItemsTrackerRef.current)');
  });

  it("重排自動關閉前先取消同類型舊 timer，避免過期回呼關閉新提醒", () => {
    expect(mapSource).toContain('clearSupplyAutoDismissTimer(type);\n        const autoDismissTimer = setTimeout(() => {');
    expect(mapSource).toContain('supplyAutoDismissTimersRef.current[type] = autoDismissTimer;');
  });
});
