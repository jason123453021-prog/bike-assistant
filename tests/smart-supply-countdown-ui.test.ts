...
const hydrationRecalculationSource = readFileSync(
  resolve(process.cwd(), "lib/hydration-recalculation.ts"),
  "utf8",
);
...
describe("smart supply countdown UI", () => {
  ...
  it("updates the paused dashboard once per second without moving the locked due point, and buffers pause recovery per channel", () => {
    expect(mapSource).toContain(
      "const [smartSupplyCountdownNowMs, setSmartSupplyCountdownNowMs]",
    );
    expect(mapSource).toContain(
      "setInterval(() => setSmartSupplyCountdownNowMs(Date.now()), 1_000)",
    );
    expect(mapSource).toContain(
      'smartSupplyCountdownRemainingSec(smartSupplyCountdown, "calorie", smartSupplyCountdownNowMs)',
    );
    expect(mapSource).toContain(
      'smartSupplyCountdownRemainingSec(smartSupplyCountdown, "water", smartSupplyCountdownNowMs)',
    );
    expect(mapSource).toContain(
      'const supplyRoundPauseRef = useRef<Record<"calorie" | "water"',
    );
  ...
  it("waits for weather and sensor inputs when starting the next smart-water round, then uses the defined safe fallback", () => {
    expect(mapSource).toContain(
      "awaitHydrationInputs({weatherPromise, sensorPromise})",
    );
    expect(mapSource).toContain(
      "void refreshSmartWaterCountdown(confirmedPlan",
    );
    expect(hydrationRecalculationSource).toContain(
      "Promise.all([input.weatherPromise, input.sensorPromise])",
    );
    expect(hydrationRecalculationSource).toContain(
      "HYDRATION_DATA_TIMEOUT_MS = 60 * 1000",
    );
    expect(hydrationRecalculationSource).toContain(
      "return MIN_WATER_COUNTDOWN_SEC",
    );
  });
  ...
  it("uses a native modal over the map and omits amount guidance", () => {
    ...
    expect(mapSource).toMatch(
      /allowSnooze=\{\s*\(!calorieAlert\s*\|\|\s*!smartEnergySupplyEnabled\)\s*&&\s*\(!waterAlert\s*\|\|\s*!smartWaterSupplyEnabled\)\s*\}/,
    );
  });
  ...
  it("provides a master supply switch that stops foreground and background reminders while preserving user preferences", () => {
    ...
    expect(mapSource).toContain(
      "settings.supplyReminderEnabled && bgState && bgState.supplyReminderEnabled !== false",
    );
  ...
  it("schedules a local Android reminder for countdown expiry and clears it only after confirmation", () => {
    ...
    expect(mapSource).toContain('scheduleSmartSupplyDueNotification("calorie"');
