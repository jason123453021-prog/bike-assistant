...
describe("Strava 公開原則的自動暫停對齊", () => {
  it("單車採 10 秒 GPS 靜止防抖，不對使用者暴露速度或秒數微調", () => {
    expect(SPORT_MODEL_PROFILES.cycling.tracking.autoPauseStillForSeconds).toBe(10);
  ...
  it("無正速度定位達 10 秒進入暫停，恢復同時需要可信位移與足夠速度", () => {
    ...
    const resumes = advanceBackgroundAutoPause({ ...noDisplacement, hasReliableMovement: true, speedKmh: 3, intervalSec: 1, enabled: true, pauseBelowKmh: 1.08, pauseAfterSec: 10, resumeAtOrAboveKmh: 1.58 });
    expect(resumes.paused).toBe(false);
  });

  it("保留公開對齊依據並清楚排除 Strava 專有程式碼宣稱", () => {
    const reference = source("references/strava-auto-pause-alignment-2026-08-24.md");
