...
const gpxContextSource = readFileSync(
  resolve(process.cwd(), "lib/gpx-context.tsx"),
  "utf8",
);
...
describe("導航與 GPX 的騎乘累計隔離", () => {
  it("釘選導航與清除導航圖層只變更導航狀態，不可開始、停止或重置騎乘", () => {
    const clearLayers = sourceBetween(
      "const clearAllNavigationLayers",
      "const startPinnedNavigationRoute",
    );
    const startPinned = sourceBetween(
      "const startPinnedNavigationRoute",
      "const selectPinAddressDestination",
    );

    for (const source of [clearLayers, startPinned]) {
      expect(source).not.toMatch(
        /dispatch\(\{\s*type:\s*"(?:START|STOP|RESET)"/,
      );
    }
    expect(clearLayers).toContain(
      'stateRef.current.status === "idle" || stateRef.current.status === "finished"',
    );
  ...
  it("清除導航圖層只移除 GPX／導航標記，必須保留正在騎乘的 live trail", () => {
    const clearStart = leafletSource.indexOf(
      "function clearNavigationGraphics(",
    );
    const clearEnd = leafletSource.indexOf(
    ...
    expect(clearGraphics).toContain(
  ...
  it("定位計算只在 active 騎乘或剛恢復的可信點後進入，避免 GPX 導航污染累計", () => {
    const locationSection = sourceBetween(
      "// 軌跡點始終記錄",
      "// ─── 騎乘計算",
    );
