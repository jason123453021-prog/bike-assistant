...
describe("Expo Go runtime 相容性", () => {
  it("僅讓 production EAS AAB 固定 runtimeVersion，避免 Expo Go 開發 bundle 與已安裝 SDK runtime 不相容", () => {
    ...
    expect(appConfigSource).toContain('version: "1.0.99"');
    expect(appConfigSource).toContain("versionCode: 10099");
    expect(appConfigSource).toContain(
      'runtimeVersion: isProductionEasBuild ? "1.0.99" : undefined',
    );
    expect(appConfigSource).not.toContain('runtimeVersion: "1.0.99",');
  });
});
