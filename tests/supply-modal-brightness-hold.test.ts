...
describe("supply modal brightness hold", () => {
  it("keeps a named brightness hold while a supply confirmation is pending", () => {
    expect(managerSource).toContain("private brightnessHolds = new Set<string>()");
  ...
  it("holds the screen for energy, water, custom, and interval supply confirmations", () => {
    expect(mapSource).toContain("const hasPendingSupplyModal = settings.supplyReminderEnabled");
    ...
    expect(mapSource).toContain("Object.values(intervalSupplyAlerts).some(Boolean)");
