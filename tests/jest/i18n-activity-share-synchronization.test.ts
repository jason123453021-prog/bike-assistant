...
import {
  generateShareCard,
  generateShareText,
} from "../../lib/garmin-card-generator";
...
describe("全域 i18n 同步、活動詳情與分享守門", () => {
  ...
  it("活動詳情、摘要與分享 Modal 均訂閱翻譯 hook，且分享 SVG 接收 locale 與 translator", () => {
    ...
    const svg = fs.readFileSync(
      path.join(rootDir, "lib/ride-share-card-svg.ts"),
      "utf8",
    );
    ...
    expect(summary).toContain("useTranslation");
    expect(summary).toContain('t("summaryDetail.coreMetrics")');
    expect(summary).toContain('t("summaryDetail.shareDistance"');
    expect(svg).toContain("options?: { t?:");
  });

  it("活動摘要新鍵在所有支援語系都可解析", async () => {
    for (const locale of SUPPORTED_LOCALES) {
      await i18n.changeLanguage(locale);
      for (const key of [
        "summaryDetail.routeName",
        "summaryDetail.coreMetrics",
        "summaryDetail.powerDistribution",
        "summaryDetail.shareDistance",
      ]) {
