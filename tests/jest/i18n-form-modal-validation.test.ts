...
describe("活動編輯與表單驗證 i18n 守門", () => {
  ...
  it("活動編輯 Modal 與補給品表單的繁中、英文文案均可即時切換", async () => {
    ...
    expect(i18n.t("forms.activityEditor.equipmentLabel")).toBe("Equipment notes");
  ...
  it("活動編輯、設定數值與自訂補給品三條表單路徑均使用翻譯 key 呈現驗證提示", () => {
    ...
    const settings = fs.readFileSync(path.join(rootDir, "app/(tabs)/settings.tsx"), "utf8");
    const supplyModal = fs.readFileSync(path.join(rootDir, "components/custom-supply-item-modal.tsx"), "utf8");
