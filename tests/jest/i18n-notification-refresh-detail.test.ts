...
describe("語言切換通知重排與活動詳情次要字串守門", () => {
  ...
  it("停用提醒或結束騎乘時不重建任何通知", () => {
    expect(buildSupplyNotificationRefreshPlan(state({ supplyReminderEnabled: false }), 10_000)).toEqual({ scheduled: [], immediate: [] });
    expect(buildSupplyNotificationRefreshPlan(state({ isRiding: false }), 10_000)).toEqual({ scheduled: [], immediate: [] });
  });

  it("指定 locale 的通知內容與 interval 提示會跟隨語言，不依賴背景預設語言", async () => {
    await i18n.changeLanguage("zh-TW");
    expect(createLocalizedSupplyNotificationContent("interval-energy-time", { intervalValue: 30 }, "en-US").body).toBe("30 minutes completed. Please refuel.");
    expect(createLocalizedSupplyNotificationContent("interval-water-distance", { intervalValue: 5 }, "zh-TW").body).toBe("已累積騎乘 5 公里，請補充水分。");
  });

  it("語言 Provider、背景通知與活動詳情皆使用可追溯的語系化入口", () => {
    const provider = fs.readFileSync(path.join(rootDir, "lib/i18n/language-provider.tsx"), "utf8");
    ...
    const deviceValidation = fs.readFileSync(path.join(rootDir, "references/android-notification-device-validation-2026-08-25.md"), "utf8");
    expect(provider).toContain("rescheduleLocalizedSupplyNotifications(nextLanguage)");
    expect(feedback).toContain("buildSupplyNotificationRefreshPlan");
    expect(background).toContain("notificationLocale");
    for (const key of ["activityAnalysis", "perKilometerSplits", "elevationBandDistribution", "exportStandardFit", "advancedTraining"]) {
      expect(detail).toContain(`t(\"detail.${key}\")`);
    }
    for (const requiredStep of ["release APK", "點擊通知本體", "已補給", "切換 App 語言"]) {
