...
## 騎乘中的 Android 系統層持續顯示（2026-08-18）
- [x] 研究 Android 縮小顯示、系統級懸浮窗與通知／PiP 的平台限制，確認 Expo Managed 可行範圍
- [x] 定義在返回主畫面或切換 App 時仍可辨識下一導航指令與核心騎乘資訊的合規呈現方式
- [x] 實作可行方案，確保開始、暫停、停止與補給狀態皆與現有騎乘資料流一致
- [x] 新增回歸測試、完成 Android 設定稽核並以實機步驟驗收
- [x] 建立最小化 Kotlin PiP Activity、Manifest 宣告與傳統 React Native 橋接；不使用 C++ 或 NitroModules
- [x] 將騎乘狀態、下一導航指令、距離、速度與暫停狀態同步為 PiP 可讀快照
- [x] 設定僅在進行中騎乘離開 App 時自動進入 PiP，並提供回到完整導航與停止騎乘的安全操作

## Android PiP 系統主題配色（2026-08-18）
- [x] 偵測 Android 系統深淺色設定，並為 PiP 定義相應背景、前景與次要文字高對比色彩
- [x] 在系統主題切換時即時重繪 PiP，不影響導航快照、騎乘統計或 PiP 進出狀態
- [x] 新增回歸測試、完成預生成檢查與實機深淺主題驗收步驟

## Android PiP 點擊返回導航（2026-08-18）
- [x] 點擊 PiP 小視窗時，將主 Activity 還原為完整單車助手導航畫面
- [x] 確保回到 App 不會停止騎乘、重設導航指令或改寫活動統計
- [x] 新增回歸測試、完成預生成與實機驗收步驟

## Android PiP 時速與轉向資訊（2026-08-18）
- [x] 將即時時速提升為 PiP 的主要可讀數據，維持深淺主題高對比
- [x] 明確標示下一轉彎方向與距離，並在無導航路線時安全顯示直行騎乘狀態
- [x] 新增回歸測試、完成預生成與實機可讀性驗收步驟
