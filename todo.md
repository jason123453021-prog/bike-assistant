...
- [x] 完成自動記圈執行層與統計稽核版本的 GitHub Actions Android APK artifact 驗證（GitHub run 32618388408）
- [x] 依實測活動與 Strava 公開計算原則校正距離、移動時間、平均速度、平均／最大功率與卡路里資料鏈；保留 30 m GPS 品質、低速連續位移、加速度抑制與虛擬峰值品質閘門
- [x] 修正 1 km 自動記圈跨里程碑計算，避免超距、0 km 分圈、極端均速與遺漏多個里程碑；前景／背景採相同固定里程插值快照
- [x] 補強背景騎乘的補給／補水到期本機通知、回到 App 的待確認彈窗與最大功率同步；背景通知使用高優先權補給頻道
- [x] 限制智慧亮度只在開始騎乘後啟動，未開始騎乘時維持系統亮度
- [x] 移除手動／自動分圈殘留說明，並將結束騎乘改回一般確認流程
- [x] 完成本輪完整 QC 與 GitHub Actions Android APK artifact 驗證（GitHub run 32615828469）
- [x] 修正開始騎乘後能量倒數與補水倒數持續顯示「計算中」而未初始化數值；開始即以本機回退模型建立倒數並同步背景狀態，GitHub run 32578986263 成功

## 專業碼表級穩定性、精準度與多運動分析優化（2026-08-22）

- [x] 稽核 GPS／海拔濾波、移動時間／自動暫停、功率／卡路里模型與多運動自動分圈的現有實作
- [x] 補強各運動自動 Lap 的專屬分段資料：單車速度／功率／爬升、跑步配速與公里分段、登山爬升／下降與 VAM；背景鎖屏期間同步持久化里程碑與 Lap 快照
- [x] 稽核導航畫面非必要浮動控制、結束騎乘防誤觸、強光高對比儀表板與四類預設收合設定介面；移除手動 Lap 後維持極簡地圖與既有高對比儀表
- [x] 優化 GPS 更新對地圖與儀表板的渲染邊界，避免高頻位置更新造成不必要的整體重繪；保留 Leaflet WebView 局部訊息更新並 memo 化儀表格
- [x] 稽核並補強背景 GPS、計時器、訂閱與本機騎乘快照的生命週期，確保長時間追蹤與離線恢復；背景與前景快照皆可恢復自動 Lap
- [x] 新增回歸測試、完成完整 QC、保存版本、同步 GitHub 並確認 Android APK artifact（GitHub run 32576375579）
- [x] 已新增背景自動分圈、長按結束確認與渲染邊界守門；完整 QC 通過（TypeScript、Lint、Vitest 111 檔／365 tests、Expo Doctor 18/18、Android Hermes export）

## 移除手動 Lap 操作並保留自動記圈（2026-08-22）

- [x] 盤點手動 Lap 按鈕、頂端提示、左上角單圈面板、設定入口與自動記圈的資料流
- [x] 移除地圖手動 Lap 懸浮按鈕及其觸控、震動與手動提示互動
- [x] 移除手動／自動模式切換與手動分圈入口，保留可持久化的自動距離記圈設定
- [x] 確保自動 Lap 仍持續寫入活動摘要、歷史活動詳情與 FIT 匯出，且導航畫面維持安靜簡潔
- [x] 更新回歸測試、完成完整 QC、保存版本、同步 GitHub 並確認 Android APK artifact（GitHub run 32573922227）

## 移除日出／日落開關燈提醒（2026-08-22）

- [x] 盤點日照提醒在設定、騎乘介面、通知排程、資料模型與測試中的相依項目
- [x] 移除設定頁的日照提醒開關、模式與提前時間控制，以及騎乘中的開關燈提醒彈窗
- [x] 移除日出／日落通知排程、背景處理與既有通知清理邏輯，保留補給與補水提醒
- [x] 清理日照設定的遺留資料讀取／寫入與相關型別，維持舊本機設定安全相容
- [x] 更新回歸測試、完成完整 QC、保存版本、同步 GitHub 並確認 Android APK artifact（GitHub run 32572330775）

## Lap 左下角控制與連續計圈穩定性（2026-08-22）

- [x] 盤點現有 Lap 控制位置、觸控範圍、資料寫入與分圈數量限制
- [x] 將 Lap 控制重構為儀表板上方的左下角 56 dp 懸浮按鈕，保留安全間距與觸控回饋
- [x] 加入 20 dp hitSlop、Pressable 觸控守門與錯誤防護，避免騎乘震動時點擊失效
- [x] 移除分圈資料陣列的上限，確保手動與自動 Lap 可連續寫入且目前單圈面板仍正常更新
- [x] 新增回歸測試、完成完整 QC、保存版本、同步 GitHub 並確認 Android APK artifact（GitHub run 32569692251）

## Lap 動態提示與地圖單圈即時面板（2026-08-22）

- [x] 盤點現有 Lap Toast、安全區、右側地圖控制與單圈即時計算資料流
- [x] 將手動 Lap 提示實作為頂端置中、安全區避讓、淡入下滑並在 3–5 秒淡出的動態 Toast
- [x] 讓 Toast 依單車、跑步／越野跑、登山顯示上一圈的專業摘要，且自動 Lap 維持靜默
- [x] 新增首次計圈後才顯示的地圖左上角單圈微型面板，依運動模式呈現圈時、功率／均速、配速或爬升
- [x] 新增回歸測試、完成完整 QC、保存版本、同步 GitHub 並確認 Android APK artifact（GitHub run 32566321214）

## Lap 提示、運動選擇與多運動分圈適配（2026-08-22）

- [x] 盤點運動選擇 Bottom Sheet、儀表板運動圖示、手動／自動 Lap 與摘要列表的狀態資料流
- [x] 修復運動選擇項目的點擊處理，使其更新運動模式、關閉選擇視窗並同步儀表板圖示
- [x] 將手動 Lap 改為僅顯示地圖頂端輕量提示；自動 Lap 不顯示干擾性提示
- [x] 讓 Lap 即時提示與摘要列表依單車、跑步／越野跑、登山呈現對應的速度／配速、功率／步頻或爬升資料
- [x] 新增回歸測試、完成完整 QC、保存版本、同步 GitHub 並確認 Android APK artifact（GitHub run 32564774608）

## 全域 Lap 設定與設定頁折疊重構（2026-08-22）

- [x] 盤點設定持久化、手動 Lap、距離累計與儀表板控制列的整合資料流
- [x] 新增可持久化的計圈主開關、手動／自動模式與 1／5／10 km 自動計圈距離設定
- [x] 整合自動距離 Lap 觸發、4 秒回饋、摘要列表與活動／FIT 共用紀錄，並在關閉時隱藏儀表板 Lap 按鈕
- [x] 將設定頁重構為騎乘與儀表板、補給與提醒、顯示與外觀、系統與資料管理四個動畫折疊面板
- [x] 新增回歸測試、完成完整 QC、保存版本、同步 GitHub 並確認 Android APK artifact（GitHub run 32559552397）

## 手動 Lap 與地圖鎖定提示修復（2026-08-22）

- [x] 盤點手動 Lap、活動儲存、騎乘摘要與 FIT 匯出的資料流，以及鎖定提示與長按進度的定位規則
- [x] 實作 Lap 快照、單圈基準重置、平均功率／速度／爬升統計與 4 秒即時回饋視窗
- [x] 在騎乘摘要新增計圈列表，並保持活動詳情與 FIT 匯出使用同一筆 Lap 資料
- [x] 分離鎖定狀態提示與長按進度按鈕的容器，移除地圖畫面排版重疊
- [x] 新增回歸測試、完成完整 QC、保存版本、同步 GitHub 並確認 Android APK artifact（GitHub run 32556090962）

## 自行車重量設定可見性核對（2026-08-22）

- [x] 核對設定頁原始碼、最新 GitHub APK artifact 與直接下載檔是否均含自行車重量欄位
- [x] 釐清並修正使用者裝置未顯示欄位的版本覆蓋或設定頁排序問題：提高 Android 版本至 1.0.4（versionCode 10088）以強制覆蓋更新
- [x] 驗證最新 APK 並提供明確的安裝更新與設定位置說明（GitHub run 32553594755；版本 1.0.4／versionCode 10088）

## 騎乘摘要與統計校正（2026-08-22）

- [x] 盤點結束騎乘、活動儲存、摘要 Modal 與重置流程中的資料快照與狀態傳遞
- [x] 修復摘要 Modal 在活動儲存／重置後讀取歸零統計的問題，保留該次完成騎乘快照至關閉摘要
- [x] 將單車自動暫停調整為低於 0.3 m/s 持續 9 秒才暫停，高於 0.5 m/s 立即恢復
- [x] 將海拔處理調整為 11 點平滑與 5 m 連續有效爬升門檻，降低 GPS 高度雜訊
- [x] 驗證修正後的移動時間、平均速度、虛擬功率與卡路里資料鏈，完成完整 QC、GitHub APK 與直接下載連結（GitHub run 32552248462）

## Android APK 直接下載交付（2026-08-22）

- [x] 下載並驗證 GitHub Actions 成功建置的 APK artifact（app-release.apk，53,121,307 bytes）
- [x] 建立可直接下載安裝的 APK 連結並交付使用方式

## 自行車重量設定（2026-08-22）

- [x] 盤點個人設定、即時騎乘、歷史正規化及路線分析中自行車重量的資料流
- [x] 新增可持久化的自行車重量設定，提供安全範圍、單位與預設 9 kg
- [x] 將使用者設定套用至即時虛擬功率、卡路里、背景追蹤與歷史活動／路線估算
- [x] 新增回歸測試、完成完整 QC、保存版本、同步 GitHub 並確認 Android APK artifact（GitHub run 32545587149）

## GPS、移動時間、爬升、功率與卡路里校正（2026-08-22）

- [x] 盤點同趟活動與 Strava 在距離、移動時間、爬升、平均功率與卡路里的差異，以及現有過濾／估算資料鏈
- [x] 研究並保存 Strava 官方公開的移動時間與 GPS 資料處理說明，以及公開可驗證的自行車物理功率與作功熱量依據
- [x] 調整 GPS 點位品質與距離累計：接受 30 m 內精度、10 秒內相鄰樣本的 Haversine 距離，同時保留防止不可信跳點的上限
- [x] 重構低速移動與自動暫停緩衝，使 0.5 m/s 以上與短暫低速仍保有合理移動時間
- [x] 以 3–5 點平滑與 3 m 有效上升門檻重建累計爬升，避免 GPS 高度雜訊
- [x] 以滾動、空氣與重力阻力的自行車物理模型重構本機虛擬功率，並以機械效率的總作功校正卡路里
- [x] 新增同趟數據校正與邊界情境回歸測試，完成完整 QC、保存版本、同步 GitHub 與確認 Android APK artifact（GitHub run 32508345264）

## 僅日出提醒與設定頁整理（2026-08-21）

- [x] 盤點現有日照提醒模式與設定頁分區、相依控制及說明層級
- [x] 新增可持久化的僅日出提醒模式，維持日出日落與僅日落模式相容
...
- [x] 新增日照提醒範圍選項，支援日出與日落或僅日落，並持久化使用者偏好
...
- [x] 新增可持久化的日照警示總開關與提前提醒分鐘數，預設維持目前提醒行為
...
- [x] 盤點最佳速度、暫停時間、補水次數、環境／智慧補給、進階訓練、表現指標與每公里功率的資料來源及遺漏原因
- [x] 研究並保存 Strava 公開活動統計與分段資料原則，以及 Garmin 手動 Lap 的公開行為規格
- [x] 修正活動詳情的最佳速度、暫停時間、補水次數、環境／智慧補給、進階訓練與表現指標顯示；資料不足時使用明確且不誤導的繁體中文狀態
- [x] 移除活動詳情中的 GPS 點數使用者可見欄位
- [x] 修正每公里分段的功率來源與顯示，避免以無效 0 W 或不可靠值誤導使用者
- [x] 修正騎乘鎖定提示圖層重疊，並在儀表板天氣列加入系統時間
...
- [x] 研究 Strava 官方公開的活動統計、海拔修正與功率／卡路里資料處理原則，保存可驗證依據
- [x] 修正平均功率無資料語意、虛擬功率計算與活動詳情顯示，避免假 0 W 或不可信估算
- [x] 修正最大坡度短距離／高程雜訊尖峰，並讓水分流失依可用資料與離線基準保守計算
- [x] 將離線回退的資料來源、估算狀態與使用者可見說明統一為繁體中文
- [x] 全專案掃描並取代問號圖示、問號回退與簡體中文字串，補足圖示映射與繁體文案
- [x] 新增統計、估算、文字與圖示回歸測試，完成完整 QC
- [x] 保存版本、同步 GitHub、確認 Android APK artifact 成功並在交付中提供網址

## 最大功率與活動詳情功能有效性 QC（2026-08-21）

- [x] 盤點最大功率估算、保存、正規化與詳情頁顯示，修復不合理極值或無效零值的語意
- [x] 逐項盤點活動詳情的路線統計、爬升、補給、環境、進階訓練與表現指標，處理無資料／不支援欄位
- [x] 讓舊活動詳情與分享／歷史路線使用相同的已正規化統計，避免錯誤原始欄位再次顯示
- [x] 更新未來交付標準：每次功能或 bug 修復後須在結果中提供成功 GitHub APK 建置連結
- [x] 建立最大功率與詳情資料有效性回歸測試，執行完整 QC
- [x] 保存版本、同步 GitHub、確認 APK artifact 成功並交付連結

## 活動紀錄、爬升與定位標記修復（2026-08-21）

- [x] 盤點靜止自動暫停、GPS 處理、背景復原與停止騎乘對活動累計的所有干涉點
- [x] 修復靜止／室內時速度與累計暫停、恢復騎乘後接續累計且不重置的狀態機
- [x] 依公開可驗證的運動記錄原則校正 GPS 海拔累積與熱量計算
- [x] 將地圖目前位置箭頭改為高可見度圓形定位標記
- [x] 新增活動統計、暫停、爬升與熱量回歸測試並完成 QC
- [x] 保存版本、同步 GitHub 並確認 Android APK artifact 成功建立
...
- [x] 移除活動詳情的「環境樣本」欄位、「本機 1 km 個人最佳比較」、「Lap 紀錄」與「本機分段功率表現」顯示區塊，保留智慧補給、每公里分段及底層活動、補給與自動分段資料供統計及匯出使用

## 個人化首輪補給與補水倒數（2026-08-23）

- [x] 開始騎乘時以可用的個人設定、運動類型、位置／速度／海拔、快取天氣與強度資料建立首輪補給／補水計畫；缺少資料時安全回退離線基準，且首輪建立後不被後續樣本重設

## 智慧補水倒數區間優化（2026-08-23）

- [x] 優化補水模型，維持汗率、強度與環境負荷的個人化調整，並將每輪補水倒數限制於 10–30 分鐘

## 摘要安全區與高濕熱補水校正（2026-08-23）

- [x] 修正活動摘要右上角關閉按鈕與 Android 系統狀態列的安全區衝突
- [x] 校正首次補水倒數，使高濕熱天氣可依體感負荷提前，而非誤落在 30 分鐘上限

## 動態補水權重與資料逾時降級（2026-08-23）

- [x] 以環境基礎區間、強度、時長與汗率重構每輪 10–30 分鐘智慧補水倒數，首輪只採環境與預設汗率
- [x] 讓補水重新計算同步等待天氣與感測資料最多 60 秒；逾時延用前一輪，首輪斷網則採 10 分鐘安全值
- [x] 新增動態區間、首輪、資料逾時與離線降級的回歸測試，完成 Android 品質驗證與 GitHub APK 建置

## 活動對齊、背景確認與觸控鎖修復（2026-08-24）

- [x] 盤點本次活動與 Strava 的距離、移動時間、均速、爬升、功率及熱量差異，校正可追溯的本機統計資料鏈
- [x] 讓背景系統補給通知的「已補給」動作直接完成確認、清理通知並建立下一輪倒數，不再等待前景彈窗二次確認
- [x] 修正觸控鎖長按中斷後殘留的百分比進度提示，並新增取消／離開／終止觸控的回歸保護

## 暫停期間真實時間補給倒數（2026-08-24）

- [x] 讓智慧能量與補水倒數以真實時間持續遞減，不受手動暫停、自動暫停或 GPS 中斷影響，且鎖定本輪到期時間
- [x] 依本輪實際暫停累積時長乘以恢復係數計算低強度／恢復權重，緩存至下一輪倒數建立時才套用，不使用固定分鐘數調整
- [x] 讓能量倒數在暫停期間維持基礎消耗，並驗證前景／背景同步與每秒平滑顯示

## 無效 GPS 速度自動暫停修復（2026-08-24）

- [x] 將前景與背景自動暫停的 null、undefined、NaN 或無定位速度正規化為 0 km/h，避免比較判定被略過
- [x] 修復起始距離為 0 時的 8 秒低速防抖，容忍短暫 GPS 飄移並於設定門檻加 0.5 km/h 時立即恢復
- [x] 新增無效速度、初始靜止、飄移與恢復門檻回歸測試，完成 QC、GitHub Android APK artifact 與實機驗收建議

## 自動暫停延遲時間自訂（2026-08-24）

- [x] 在設定頁新增可選的自動暫停延遲時間，預設維持 8 秒並保存使用者偏好
- [x] 讓前景無 GPS 看門狗與背景自動暫停共用使用者指定的延遲時間
- [x] 新增設定持久化、邊界回退與前背景延遲一致性測試，完成 QC、GitHub Android APK artifact 與實機驗收建議

## Strava 風格自動暫停自動化（2026-08-24）

- [x] 查核 Strava 公開自動暫停原則並保存可追溯依據，不宣稱取得其專有底層程式碼
- [x] 以本機運動類型策略自動決定暫停門檻與延遲，移除設定頁不必要的手動速度門檻與延遲調整
- [x] 維持低速／無 GPS 防抖與恢復遲滯，新增對齊行為回歸測試，完成 QC、GitHub Android APK artifact 與實機驗收建議

## 活動詳情自動暫停統計（2026-08-24）

- [x] 在騎乘資料鏈持久化自動暫停總時間，與手動暫停時間分開計算且避免重複累計
- [x] 在活動紀錄詳細資料顯示自動暫停總時間與清楚的統計語意
- [x] 新增開始／恢復／停止、儲存與舊紀錄回退測試，完成 QC、GitHub Android APK artifact 與實機驗收建議

## 活動資料完整性與 COG 軌跡導航重構（2026-08-24）

- [x] 盤點並修復移動時間由暫停狀態干擾、造成平均速度與功率異常的資料鏈；以可信 GPS 時間戳／位移重新計算並加入防爆守門
- [x] 確保進行中活動的可信 GPS 點持續保存與標準 GPX／FIT 匯出，不因自動暫停而任意遺失原始時間戳座標
...
## 完整功能地圖與產品規格書（2026-08-24）

- [x] 深度掃描 App、核心函式庫、匯出器、設定、恢復機制與測試，建立僅依目前代碼庫的功能盤點
...
## i18n 體驗、歷史／路線與 RTL 擴充（2026-08-25）

- [x] 盤點語言切換生命週期、歷史與路線頁字串，以及 RTL 相容性風險
...
## 活動通知、匯出檔名與術語校對（2026-08-25）

- [x] 盤點活動詳情通知、GPX／FIT／分享檔案命名與現有單車術語的語系來源
...
- [x] 以 GitHub Android Emulator 自行驗證背景通知回前景 action、RTL 與 200% 字體大小流程並檢視 artifact（run 32841510977、head `ba2c494e`；核心 1／0、通知排程 1／0、通知本體回前景 1／0、130% 與 200% 各 3／0 failures；已檢視 Refuel Modal 與 200% Arabic RTL artifact）
...
- [x] 修正設定頁「系統與資料管理」中實機 1.0.90 未顯示語言切換入口的可見性缺陷，並重新建置驗收 APK（移至系統權限卡後、清除快取前；1.0.91／10091，APK run 32848127211、原生 E2E run 32848127161 均成功）
- [x] 移除上方轉彎提示與語音提示，僅依實際連續騎乘軌跡平滑調整地圖轉向並維持行進方向朝前
- [x] 將右上角定位按鈕重構為 COG 朝前、自由角度、正北置中三階段循環，使用已映射的非問號圖示
- [x] 實作三種定位模式共用的手動平移／旋轉暫停與閒置後依當前模式置中歸位邏輯
- [x] 騎乘完成存檔後清除當次軌跡線、路線標記與暫存地圖圖層
- [x] 移除設定頁「清理地圖」及「清理暫存軌跡」入口與相關 UI 測試期望
- [x] 強化 COG 地圖旋轉的抗抖動平滑策略，避免 GPS 訊號波動造成頻繁旋轉或抖動（8° 死區、45° 單次候選步幅與 0.42 指數平滑）
- [x] 在設定頁新增可本機保存的「閒置歸位時間」選項，套用於手動平移／旋轉後的自動置中計時（3–60 秒滑桿；既有設定安全載入與保存）
- [x] 以最新 COG 平滑與閒置歸位版本建立可安裝 Android APK，核對產物後提供直接下載網址（1.0.92／versionCode 10092；GitHub run 32863812298，artifact 9569908341，28,463,758 bytes，未過期）
- [x] 提升地圖右上角自由定位模式按鈕的實心背景、陰影層次與非高亮圖示對比，確保複雜地圖與戶外光線下可辨識
- [x] 盤點並補齊儀表板、進階儀表板、設定頁與路線分析的 i18n 字串，讓 12 種支援語系可即時完整切換
- [x] 新增硬編碼字串掃描與 12 語系核心畫面回歸守門，完成完整 QC 與最新版 Android APK artifact（1.0.93／versionCode 10093；GitHub run 32869917632，artifact 9572317313，28,482,033 bytes，未過期）
- [x] 深度清查設定頁與設定關聯元件的硬編碼使用者文案，涵蓋儀表板、地圖、顯示省電、補給回饋、背景 GPS、個人資料與 App 自動推定
- [x] 以 13 語系翻譯鍵與 i18next 插值替換設定頁動態句子，禁止以字串串接產生使用者可見文案
- [x] 新增設定頁硬編碼掃描、13 語系鍵完整性與插值回歸守門，完成 QC 並建置新版 Android APK artifact（1.0.94／versionCode 10094；GitHub run 32914460949，artifact 9588063846，28,487,923 bytes，未過期）
- [x] 重新校正補水倒數：僅依溫度與濕度對應設定的安全時間範圍，移除強度、暫停與體感衍生因素對補水間隔的影響
- [x] 修正省電喚醒亮度、前景沉浸式導航列、觸控鎖的閒置上鎖與幽靈長按進度清理；Android 喚醒會撤銷 activity 亮度覆寫交還系統／自動亮度，沉浸式與手勢導航的實機結果仍受 OEM 與系統模式限制
- [x] 讓釘選導航開始後立即回到當前 GPS 位置，並統一即時坡度、總爬升、活動儲存與匯出資料鏈
- [x] 新增可重現回歸測試、完成完整品質驗證並建置新版 Android APK artifact（1.0.95／versionCode 10095；GitHub run 32921525927，artifact 9590266903，28,499,404 bytes，未過期；Jest、Vitest 127 檔／444 tests、TypeScript、Lint、Expo config 與差異檢查均通過）
- [x] 將智慧補水下一輪排程恢復為 FTP、體重、強度、時長、坡度、環境與暫停因素的建議值，但依溫濕度帶嚴格箝制在 10–15、15–20 或 20–30 分鐘安全區間，並採線性過渡與對應單次飲水／電解質建議
- [x] 移除暫停時長對下一輪能量倒數的補償與重排影響，維持能量倒數的絕對時間計時
- [x] 回歸驗證 Android 喚醒交還系統／自動亮度、前景沉浸式系統列、依設定自動觸控鎖與無 pointer 時的幽靈進度清理
- [x] 回歸驗證釘選導航啟動後 GPS 視角歸位，以及較敏感即時坡度與即時／儲存／匯出爬升資料鏈一致性
...
- [x] 依 AST 結果抽取活動詳情、圖表、簡化導航覆蓋層與地圖恢復／釘選控制的可見硬編碼字串
- [x] 抽取錯誤頁、Web 地圖替代頁、隱私頁及其餘輔助元件的可見硬編碼字串
- [x] 驗證新增 i18n 鍵在 13 語系均可解析；缺失鍵僅依繁中後英文 fallback，不跨語系洩漏
- [x] 擴充 AST 掃描回歸守門，完成全套品質驗證與新版 Android APK artifact
...
## 免費補水點與拍照景點圖層（設定頁整合，2026-08-26）

- [x] 盤點 Leaflet 地圖、設定持久化、既有 POI 顯示及釘選導航的資料與互動邊界
- [x] 建立 Local-first 的補水點／拍照景點 POI 資料模型、距離與縮放聚合工具，避免地圖低縮放層級的 Marker 壅塞
...
- [x] 將使用者提供的地圖、導航、POI、補給、數據、GPX、系統控制與 i18n 清單逐項映射至可執行測試與人工驗收限制
...
- [x] 由產品負責人確認接受 1.0.102 停止支援 389 部既有支援裝置的唯一 Play pre-check 警告
...
- [ ] 逐項核對 Google Play Data Safety、現行隱私政策、敏感權限與 Default zh-TW 商店資訊是否符合 Local-first 實作；法務判斷仍須由合格律師確認
- [x] 讀取 Play Console 商店資訊草稿的審查流程與 1.0.102／10102 production 發布的關係：Console 已顯示 1.0.102／10102 在 172 地區以 100% 上架；商店資訊草稿仍須另行提交
- [x] 在送交商店資訊變更審查前，向使用者說明精確提交內容及其會建立獨立 Google 審查的後果，取得最終明確確認
- [x] 依最終明確確認送交 Default zh-TW 商店資訊變更審查；Console 已顯示「審查中的變更」，且不變更既有 production 1.0.102／10102
- [x] 記錄此次商店資訊審查狀態、完成差異檢查與 checkpoint；Default zh-TW 四項變更已顯示「審查中的變更」，Data Safety 僅儲存草稿
- [x] 依 1.0.102 現行 Local-first 實作修正公開隱私權政策源碼：補足 POI 可視範圍查詢、最多 30 分鐘本機快取、線上地圖／天氣與使用者主動媒體／GPX 處理界線
- [x] 建立 Data Safety 答案對照草案，標示位置、媒體、GPX、公開服務／WebView 與第三方 SDK 的待確認資料流，並標記沒有實作依據的帳戶／郵件／個人資訊舊聲明為待移除
- [ ] 以公開網站與程式設定核對隱私權政策改寫內容，完成 TypeScript、Lint 與差異檢查；法律充分性仍待合格律師審閱
- [ ] 建立可回溯 checkpoint，並在修改或送出 Data Safety、送審 Default zh-TW 商店資訊前取得精確的使用者提交確認
- [ ] 依使用者最終明確確認更新並提交 Data Safety、送交商店資訊變更審查，讀取 Google Play 回應且不變更已上架的 1.0.102／10102 production 配置
- [ ] 以公開部署的 2026-08-27 隱私權政策、應用程式實作與 Play Console 現有 Data Safety 逐項核對收集／分享、用途、可選性、處理方式與保留期間
- [x] 依核對結果建立逐題 Data Safety 修正表及商店資訊送審範圍，明確區分可由程式佐證、需要第三方服務確認及需要律師確認的內容
- [x] 送出 Default zh-TW 商店資訊審查前，取得精確提交內容與後果的最終使用者確認；Data Safety 僅確認儲存草稿，未確認最終提交
- [ ] 依確認完成 Data Safety 草稿更新、商店資訊審查提交、Console 結果記錄與 checkpoint；不改動已上架 1.0.102／10102 的 production 軌道配置
...
- [x] 修正進行中騎乘、匯入 GPX 與釘選目標時的 POI 圖層持續顯示，確保補水點與拍照點依設定及地圖範圍即時更新
- [x] 移除 Carto 底圖未設定 API key 時顯示的「API KEY REQUIRED」浮水印，改用無須該 key 且具道路名稱的合規底圖來源與離線容錯
- [x] 修正未存在有效觸控指標時仍顯示「100% 按住中」的幽靈長按狀態，並加入回歸守門
- [x] 修正騎乘結束按鈕及相關動作的繁中用詞，避免不符合產品術語或流程的文字
- [x] 將所有使用者可見的產品名稱統一為「單車助手」，修正「單車助理」殘留
- [x] 修正「騎程次數」等錯別字，全面盤點並校對全 App 使用者可見文字及繁中 i18n 字典
- [x] 完成修正後執行本機 QC、提高 Android 版本並以受保護 upload key 產生新的 AAB；Google Play 寫入或送審前取得精確確認（1.0.103／10103；GitHub run 33064135669 成功；SHA-256 `ce36e4c9f4b1d9506671b9789a13422d4831dd1599952e20725e0a577d81707a`）
- [ ] 取得使用者對 1.0.103／10103 Google Play 寫入範圍的確認：僅上傳並儲存正式版草稿，或上傳後送交 Google 審查；不處理未完成的 Data Safety 草稿
- [x] 唯讀檢查 Google Play 正式版頁與版本資訊主頁的建議行動，整理可優化項目；此步驟不寫入、上傳、儲存或送審任何變更
...
- [x] Google Play 拒絕 1.0.104／10104：錯誤為「版本代碼 10104 已經使用過了」；已將 Android versionCode 遞增為 10105（版本 1.0.105）、重新執行 QC 與受保護 AAB 建置並完成上傳

- [x] Google Play 已拒絕 1.0.104／10104，原因是 versionCode 已使用；已改用 1.0.105／10105 完成本機 QC、受保護 AAB 建置、上傳、pre-check 與送審

- [x] Play Console 上傳頁檔案選擇器無法由自動化定位，需重新觸發「上傳」並確認 1.0.105／10105 bundle 被接收

- [x] 稽核 Google Play 1.0.105 的 Android 15 無邊框淘汰 API、遠端點陣圖解碼與 R8／資源縮減警示，區分可由專案設定修正與上游原生相依套件訊號
- [ ] 依稽核結果修正可控的 Android 原生設定與遠端圖片載入路徑，建立遞增正式版 AAB 並重新上傳 Google Play
- [x] 查明 Google Play 對 1.0.105／10105 仍回報最高 target API 35 的實際 AAB／Gradle 組態來源，確保新 AAB manifest 可驗證為 target SDK 36 以上
