...
- [x] 重構活動摘要首屏：縮短地圖高度，讓距離／移動時間／平均速度在不捲動時可完整閱讀，並統一數字與單位基線；三項核心數據採綠色第一層、固定數字字形與單位 baseline
- [x] 強化活動摘要地圖的起點、終點與行進方向語意，並移除「全螢幕路線」控制；起點為綠色圓形、終點為紅色方形、行進方向為深色箭頭
- [x] 將活動詳情地圖高度、核心統計排序與數字／單位排版回復至活動摘要首屏重構前的樣式，保留「全螢幕路線」控制移除；活動地圖與兩欄摘要皆已回復、路線端點語意維持改善
