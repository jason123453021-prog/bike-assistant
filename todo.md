...
## 高級導航功能實現（v3.0）
### 第一階段：頂部實時轉彎提示導航欄
- [x] 建立 NavigationBar 組件（components/navigation-bar.tsx）
- [x] 實現深綠色（#1B5E20）背景與圓角設計
- [x] 動態轉彎圖示映射（左轉、右轉、直行、U 轉等）
- [x] 導航指令與剩餘距離顯示
- [x] 未導航時自動隱藏或顯示引導文字
- [ ] 集成到 map.tsx 頂部

### 第二階段：地圖路徑箭頭指引
- [x] 實現 Douglas-Peucker 演算法軌跡壓縮
- [x] 計算箭頭位置（每 100 公尺間隔）
- [x] 實現白色箭頭渲染（> 或 >> 符號）
- [x] 動態調整箭頭密度（基於地圖縮放級別）
- [ ] 優化性能，確保地圖放大縮小不卡頓
...
### 第三階段：語音導航與節能模式
- [x] 實現逐向語音導航（播報轉彎、盲區、危險路段）
- [x] 集成 expo-speech 進行中文語音播報
- [x] 實現智慧節能模式（長直路自動變暗螢幕）
- [ ] 在設定頁面新增節能模式開關（自動/手動/關閉）
- [ ] 實現螢幕亮度動態調整

### 第四階段：地址規劃與路徑切換
- [ ] 在導航頁面新增地址輸入框
- [x] 集成地理編碼 API（地址 → 座標）
- [x] 實現路由規劃（OSRM 或 Google Maps API）
- [ ] 支持自行車模式與一般道路模式切換
- [ ] 顯示多條路線選項供用戶選擇

### 第五階段：行徑方向箭頭
- [x] 實現當前位置方向指示（基於 GPS 方向角）
- [x] 在地圖上疊加方向箭頭
- [x] 箭頭顏色與大小動態調整（基於速度）
- [x] 低速時箭頭變淡，停止時隱藏

### 第六階段：測試與驗收
- [ ] 全功能 TypeScript 編譯檢查（0 errors）
- [ ] 防遮擋自適應測試（面板收縮/展開）
- [ ] 防呆與狀態一致性測試（設定變更後同步更新）
- [ ] 在實際設備上測試所有功能
- [ ] 性能優化與記憶體洩漏檢查
- [ ] 保存檢查點
