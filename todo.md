...
- [ ] 盤點 Leaflet 地圖、設定持久化、既有 POI 顯示及釘選導航的資料與互動邊界
- [ ] 建立 Local-first 的補水點／拍照景點 POI 資料模型、距離與縮放聚合工具，避免地圖低縮放層級的 Marker 壅塞
- [ ] 在設定頁新增並持久化「顯示免費補水點」及「顯示拍照景點」開關，不在地圖主畫面新增控制按鈕
- [ ] 實作高對比水滴／相機 Marker、點擊資訊卡、選用預覽與「設為導航點／釘選」既有路線整合
- [ ] 補齊所有支援語系的設定、類別、資訊卡與導航動作文案，並新增單元／E2E 回歸與 Android APK 驗收
