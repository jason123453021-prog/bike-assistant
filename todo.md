...
## 發布前品質管制與功能有效性稽核（2026-08-18）
- [x] 建立本輪品管基準，盤點既有測試、版本配置、未提交變更與已知實機限制
- [x] 稽核活動統計、GPS 軌跡、背景復原、定位／計時器監聽與本機持久化生命週期
- [x] 盤點設定、按鈕、權限、通知與背景服務是否存在「有介面但未接入實際資料流」的功能
- [x] 修復已發現的空白地圖按鈕處理器與分享卡複製功能殘留，補足使用者回饋與回歸測試
- [x] 移除無實際功能接入且永遠判定未授權的 Android 懸浮窗權限與假性引導
- [x] 稽核純離線回退、螢幕安全區、防誤觸、Expo Go 邊界、發布設定、套件與除錯殘留
- [x] 修正所有可重現問題並補足回歸測試
- [x] 執行完整型別、lint、測試、Expo Doctor、Android Hermes 匯出與發布前設定驗證
- [x] 產出發布前品管檢驗清單、保存版本並交付結果

## 移除 Android PiP／縮小懸浮視窗（2026-08-18）
- [x] 已回滾至未加入 PiP 的發布前品管版本，保留騎乘、導航、補給、設定與分享功能
- [x] 已驗證正式程式、設定與測試未保留 PiP、系統級懸浮窗或相關原生橋接

## 回滾後全機發布前品質管制（2026-08-18）
- [x] 建立回滾後測試、發布設定與已知實機限制的品質基準
- [x] 稽核活動統計、GPS 漂移／中斷、背景復原、持久化批次與資源釋放
- [x] 稽核定位／通知權限防呆、離線回退、螢幕安全區、防誤觸與有效性不足的控制項
- [x] 稽核版本、圖示、Android API 36、R8、依賴、原生風險與正式路徑除錯殘留
- [x] 修復可重現問題、補足回歸並完成完整型別、Lint、Expo Doctor、Android bundle 預檢
- [x] 產出發布前品管檢驗清單、保存版本與交付實機驗收重點
- [x] 清除離線或路線端點失敗時的路由服務開發日誌，維持既有安全 null 回退
- [x] 將釘選路線的離線或端點失敗由警示彈窗改為同頁非阻斷說明
- [x] 移除釘選路線規劃的額外語音，維持僅補給／補水固定短句的語音政策
- [x] 清除騎乘、背景定位與省電熱路徑的開發型警告日誌，維持既有安全回退
- [x] 移除自動暫停／恢復的額外語音，維持僅補給／補水固定短句的語音政策
- [x] 清理路線語音移除後的未使用匯入，恢復零 Lint 警告
- [x] 移除剩餘導航、到達與移動狀態語音，維持僅補給／補水固定短句的語音政策
- [x] 清理語音政策調整後無效的 Hook 依賴，恢復零 Lint 警告

## 修復重設所有設定未生效（2026-08-18）
- [x] 重現並檢查重設按鈕、二次確認、SettingsContext 與 AsyncStorage 的資料流
- [x] 修復重設後的設定狀態、儀表板欄位與畫面同步，且不刪除活動、軌跡與相片
- [x] 新增回歸測試並以實機步驟驗收重設預設值是否立即可見
