...
- [ ] 重新校正補水倒數：僅依溫度與濕度對應設定的安全時間範圍，移除強度、暫停與體感衍生因素對補水間隔的影響
- [ ] 修正省電喚醒亮度、前景沉浸式導航列、觸控鎖的閒置上鎖與幽靈長按進度清理
- [ ] 讓釘選導航開始後立即回到當前 GPS 位置，並統一即時坡度、總爬升、活動儲存與匯出資料鏈
- [ ] 新增可重現回歸測試、完成完整品質驗證並建置新版 Android APK artifact
